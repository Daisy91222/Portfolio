...
opa <- st_read("data/opa_properties_public.geojson", quiet = TRUE)
```
...
# Build spatial weights matrix using K-nearest neighbors (k=5), row-standardized
coords <- st_centroid(phl_props_filtered) %>% st_coordinates()
lw     <- nb2listw(knn2nb(knearneigh(coords, k = 5)), style = "W")

# Global Moran's I test
moran_global <- moran.test(phl_props_filtered$log_sale_price, listw = lw, zero.policy = TRUE)
print(moran_global)

# Moran scatter plot: x = log_sale_price, y = spatially lagged values
moran.plot(phl_props_filtered$log_sale_price, listw = lw,
           main = "Moran's I Scatter Plot - log_sale_price",
           xlab = "log_sale_price", ylab = "Spatially Lagged log_sale_price")

# Local Moran's I (LISA)
lisa <- localmoran(phl_props_filtered$log_sale_price, listw = lw, zero.policy = TRUE)

# Append LISA statistics and classify each observation into a cluster type
...
ggplot(phl_props_filtered) +
  ...
  labs(title = "LISA Clusters - log_sale_price") +
  theme_minimal()
...
# -----------------------------------------------------------
# EDA plots (likely to reuse in slides)
# -----------------------------------------------------------
saveRDS(p1, "data/p1_transit.rds")    # Transit stops vs log price
saveRDS(p2, "data/p2_trees.rds")      # Street trees vs log price
saveRDS(p3, "data/p3_schools.rds")    # School distance vs log price
saveRDS(p4, "data/p4_distcore.rds")   # Distance to Center City vs log price
